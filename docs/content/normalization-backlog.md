...
- **LB1. [CODE DONE `d0ff716` — awaits Matt's dashboard toggle]** Re-enable Supabase "Confirm email" before real parent signups (was OFF 2026-06-13 for live testing; unconfirmed-email signups on parent-held minor accounts are a COPPA/security gap). The APP now handles confirmation-on: signUp returns a pending-confirmation state ("Check your email" panel) instead of a bad redirect, sign-in shows a specific unconfirmed hint, and /auth/sign-in?confirmed=1 shows a success banner. **Matt's remaining (dashboard, no code):** flip "Confirm email" ON; set Site URL + redirect to /auth/sign-in?confirmed=1 + the confirmation email template. Pairs with the broader COPPA consent-method counsel item. No "resend link" in v1 (deferred).
- **LB3. [RESOLVED 2026-06-14, commit `725ee23`]** ~~Diagnostic renders `choice`-kind items as a free-text box (mis-places students).~~ FIXED: new `components/ui/ChoiceInput.tsx` radiogroup widget; DiagnosticFlow + PracticeFlow branch on `answer.kind` via `widgetForKind()` (choice→ChoiceInput, else text Input); practice served DTO threads `choices`; **51 ALG-E14 items** (p1/p2/p3) given their `choices[]` (data was the only DATA defect — root cause was RENDER); new validator choice-integrity rule in `lib/validation/index.ts` (choice⇒choices[]≥2, value∈choices, no dup, ≥1 distractor, map-keys∈choices) so the class can't recur. 662 tests, graph VALID, checkAnswer/engine byte-unchanged. Live-verified: choice items now render as selectable options in the diagnostic; correct selection scores correct. Original analysis kept below for context.
  _(original)_ Diagnostic renders `choice`-kind items as a free-text box (mis-places students). Found in live verification 2026-06-14: the placement diagnostic shows a "Type your answer" textbox for EVERY item, including `kind: choice` problems (e.g. ALG-S04 "no solution", ALG-P09 "x^2 + 6x + 9", ALG-Q08 "x = 6 or x = −6", ALG-S03 a long concept sentence). A student must blindly type the EXACT canonical choice string or be marked wrong — mathematically-correct answers fail on whitespace/format ("x^2+6x+9" vs "x^2 + 6x + 9"). Net effect: every real student is mis-placed "developing" on the choice items (drove the bogus Systems/Exponents/Quadratics "developing" estimates in the test run). FIX before real students: render `choice` diagnostic items as selectable options (radio/buttons), OR have the diagnostic draw only free-response items. Gated: mr-kahn (diagnostic/curriculum) + pee-wee (UI). Relatedly, the practice/diagnostic exact-string checker whitespace-sensitivity is the existing A2 "checker ergonomics" theme.
...
- **LB2. [RESOLVED `8f8f961`]** ~~FERPA read-audit wiring~~ (Phase 11B §B12). DONE: `appendAccessLog` on A3Repository (Supabase userClient INSERT under the actor-bound RLS; InMemory no-op); the admin per-student record view (/admin/students/[studentId]) now HARD-GATES on a persisted `record_access_log` row (inserted BEFORE the record reads; on failure → notFound, no unaudited disclosure). Logs human inspection only (`student_insight`); engine/service-role/system reads excluded by design. Residuals (tracked): roster un-logged (list metadata); parent record-read logging deferred until a parent per-child record view exists (must log `parent_child_record` before shipping); transcript surface logs `transcript` when staff/parent-facing.
