...
- **`elements[].counteredEntryIds`** — the taxonomy entries this element counters (taxonomy §3.3 annotation). **Host-side only** — consumed by the deterministic engine for the missing-element prior-raise; never sent to the grader and never echoed in grader output. **Declared deviation / proposed amendment to RUNTIME_TUTOR_SPEC §2:** the RUNTIME §2 context-envelope example currently carries `counteredEntryIds` inside `item.rubric` on grading calls; this contract's never-sent rule is a stricter anti-bias design (the grader must judge element presence without knowing which beliefs an element is meant to counter). It is flagged here as a deviation from the consumer spec pending that amendment, not asserted as compatible.
