...
| `GENERATION_MODEL_ALT` | `claude-sonnet-5` | $1.50 / $7.50 (intro $1.00 / $5.00 through 2026-08-31) | Cost-down pilot candidate (§1.1) |
...
7. **[SEPARATE GATED TASK — NOT THIS SPEC]** Promotion of approved nodes from `.authoring-tmp/regen/` into `data/algebra1-graph.json` / Supabase. Requires mr-gates (schema/migration) + mr-kahn (content) + Matt (commit). This document's authority ends at staging. **Two gated preconditions must clear before any promotion (Matt, 2026-07-03):**
   ...
   - **(7b) [GATE — mr-gates] — OPEN; target updated 2026-07-06 to the promotion-time pinned version** (was fixed `1.11.0`, then `1.12.0`). **Supabase reconciliation to the promotion-time pinned graph version must complete** before any regen output promotes — the pin rolls forward with gated graph-mutation batches (§7 F-DEP), so a fixed version target goes stale the moment a taxonomy batch's approved diffs apply. The active/published graph is currently `1.9.2`; promoting regen output onto a `1.9.2`-active Supabase would create a graph/runtime mismatch. Reconcile Supabase to the pinned git baseline first. *(2026-07-06 amendment per TAXONOMY_REGEN_SPEC gate review, mr-kahn.)*
...
| Registry state | Pinned per bank batch to the latest gated graph-mutation application state — a monotone superset of `1.12.0`'s 163 entries; bank batch k renders only after the gated batch applying taxonomy batch k's approved diffs (§7 F-DEP; mirrors TAXONOMY_REGEN_SPEC §8.1). F-DEP hard-stops if pre-diff or off the batch's pin. *(2026-07-06 amendment per TAXONOMY_REGEN_SPEC gate review, mr-kahn.)* |
| Graph baseline | Pinned per bank batch to the same gated application state (git `1.12.0` at consolidation; grows only via gated graph-mutation batches — §7 F-DEP; promotion targets the promotion-time pin, §8 gate 7b) |
