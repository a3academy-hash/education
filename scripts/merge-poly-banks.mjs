// scripts/merge-poly-banks.mjs
// One-off content-debt batch 5 merge (mr-kahn authored and approved all content):
// merges the 9 exponents-polynomials-domain problem-bank fragments in
// .authoring-tmp/ALG-P01.json … ALG-P09.json into data/algebra1-graph.json, then
// bumps schema.version to 1.7.0. Every fragment is validated before anything is
...
// Run once: node scripts/merge-poly-banks.mjs
...
const ACCEPTED_VERSIONS = new Set(["1.6.0"]);
...
for (const id of fragmentIds) {
  ...
  for (const [bucket, phase] of [["p1", 1], ["p2", 2], ["p3", 3]]) {
    ...
    for (const p of problems) {
      ...
      // allowed (e.g. ALG-P02 quotient-rule items).
...
console.log(
  `Merged ${fragments.length} exponents-polynomials banks (${totalProblems} problems, ` +
