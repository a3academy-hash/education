// scripts/merge-data-banks.mjs
// One-off content-debt batch 7 merge (mr-kahn authored and approved all content):
// merges the 4 data-domain problem-bank fragments in
// .authoring-tmp/ALG-D01.json … ALG-D04.json into data/algebra1-graph.json, then
// bumps schema.version to 1.9.0. This is the FINAL content domain — it completes
// the Algebra 1 graph. Every fragment is validated before anything is assigned;
// any failure aborts the whole run before the graph file is written.
// Fragments are left in place as the audit trail until commit.
//
// Conventions (same as the Quadratics batch):
//   1. answer.kind may be "numeric" or "choice". Non-choice items carry no choices.
...
const ANSWER_KINDS = new Set(["numeric", "choice"]);

const ACCEPTED_VERSIONS = new Set(["1.8.0"]);
const TARGET_VERSION = "1.9.0";
  ...
  for (const [bucket, phase] of [["p1", 1], ["p2", 2], ["p3", 3]]) {
    ...
    for (const p of problems) {
      ...
      if (!ANSWER_KINDS.has(p.answer?.kind)) {
        fail(id, `${bucket} problem ${p.id}: answer.kind "${p.answer?.kind}" not in {numeric, choice}`);
      }
      if (typeof p.answer?.value !== "string") {
        fail(id, `${bucket} problem ${p.id}: answer.value is not a string`);
      }
      if (p.answer?.choices !== undefined || p.answer?.misconceptionMap !== undefined) {
        fail(id, `${bucket} problem ${p.id}: choices/misconceptionMap nested inside answer`);
      }
...
console.log(
  `Merged ${fragments.length} data banks (${totalProblems} problems, ` +
