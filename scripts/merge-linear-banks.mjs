// scripts/merge-linear-banks.mjs
// One-off content-debt batch 3 merge (mr-kahn authored and approved all content):
// merges the 18 linear-functions problem-bank fragments in .authoring-tmp/ALG-L01.json
// … ALG-L18.json into data/algebra1-graph.json, then bumps schema.version to 1.5.0.
...
// Run once: node scripts/merge-linear-banks.mjs
...
const ACCEPTED_VERSIONS = new Set(["1.4.1"]);
...
for (const id of fragmentIds) {
  ...
  for (const [bucket, phase] of [["p1", 1], ["p2", 2], ["p3", 3]]) {
    ...
    for (const p of problems) {
      ...
      // answer.value and every misconceptionMap key.
...
console.log(
  `Merged ${fragments.length} linear banks (${totalProblems} problems, ` +
